...
namespace CecilMixerTest
{
    ...
    public class ReferenceAssemblyTest
    {
        ...
        {
            Assert.AreEqual(expected.Name, actual.Name, string.Format("Looking for type {0} but found {1} in mixed assembly", expected.Name, actual.Name));
        ...
        private static void AssertEquality(MethodDefinition expected, MethodDefinition actual)
        {
            ...
            if (expected != null && actual == null)
            {
                Assert.Fail(string.Format("Expected method {0} in type {1} but found none of mixed assembly", actual.FullName, actual.DeclaringType));
            }
